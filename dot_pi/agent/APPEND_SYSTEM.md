...
- Default to direct tools for small, clear, localized changes (configuration, comments, simple fixes) and their verification. A bounded task is not by itself a reason to delegate. Use subagents when independent investigation, parallel work, substantial execution, or an independent review clearly outweighs handoff overhead, or when the user explicitly requests them. Once delegation is justified, choose Implement for bounded changes; use general-purpose only when investigation and execution are genuinely inseparable.
...
- Agent runs are bounded by the repository defaults `defaultMaxTurns: 640`, `defaultMaxToolCalls: 1024`, `defaultMaxTokens: 3000000`, and `graceTurns: 8`; omit `max_turns` so those defaults apply. Only request a smaller explicit `max_turns` for unusually narrow tasks; never pass `0` or a larger value to disable or weaken the safety budget.
- Use dynamic workflows for broad decomposable work that benefits from parallel independent evidence, not routine edits. Keep one writer in a shared checkout, use the configured token budget, and do not nest Agent delegation inside workflow children. A workflow `agent()` call does **not** name a catalogue key: pass `strength` with `low`, `medium`, or `high`, which pi-workflows maps onto a tier itself. `tier`, `model`, and `thinking` are rejected there — as is any other option name `agent()` does not read, so pass only documented keys. Omitting `strength` sends no tier and runs at `medium`; with no `strengths` table configured, each strength runs on the tier of the same name.
