...
<p>
  <img src="docs/images/preview.png" alt="Desktop preview — Ghostty terminal with AeroSpace tiling and a fastfetch system summary" width="400" />
  <img src="docs/images/herdr-lazyvim-preview.png" alt="Herdr workspace preview — LazyVim in Ghostty with repository and agent status sidebars" width="400" />
</p>
...
They are normalized into `~/.harnesses/skills` as a shared library; use `skill-activate` from a project directory to curate active symlinks in `./.claude/skills`, `./.codex/skills`, `./.pi/skills`, and `./.cursor/skills` (Cursor CLI reads `./.cursor/skills` natively and follows symlinked skill folders). `skill-activate --category typescript` activates a whole category for the current directory, `skill-activate --sync` repairs partial pairs across the harness dirs, and `Ctrl-A` toggles the highlighted category in the picker.
