   ...
   class MigrationManager {
     ...
     async getPendingMigrations() {
       ...
       const executedVersions = new Set(
         executedMigrations.rows.map((row) => row.version),
       );
     ...
     async runMigration(migrationFile) {
       ...
       try {
         ...
         const checksum = this.generateChecksum(
           migrationContent || migrationFile.file,
         );
         ...
         console.log(
           `✓ Migration ${migrationFile.file} completed in ${executionTime}ms`,
         );
       } catch (error) {
         await client.query("ROLLBACK");
         console.error(
           `✗ Migration ${migrationFile.file} failed:`,
           error.message,
         );
         throw error;
       } finally {
     ...
     async rollbackMigration(migration) {
       ...
       try {
         ...
         await client.query("ROLLBACK");
         console.error(
           `✗ Rollback failed for ${migration.name}:`,
           error.message,
         );
         throw error;
       } finally {
...
4. **Advanced Migration Patterns**
   ...
   **Data Migration with Validation:**
   ...
   const migration = {
     ...
     description:
       "Transform legacy user_settings JSONB column to normalized user_preferences table",
     ...
     async validateMigration(client) {
       ...
       if (oldCount.rows[0].count !== newCount.rows[0].count) {
         errors.push(
           `User count mismatch: ${oldCount.rows[0].count} vs ${newCount.rows[0].count}`,
         );
       }
   ...
   class MigrationTester {
     ...
     async setupTestDatabase() {
       ...
       try {
         await adminPool.query(
           `DROP DATABASE IF EXISTS ${this.testDbConfig.database}`,
         );
       ...
       for (const query of queries) {
         ...
         try {
           ...
           if (duration > (query.maxDuration || 1000)) {
             console.warn(
               `⚠ Query ${query.name} took ${duration}ms (expected < ${query.maxDuration || 1000}ms)`,
             );
           }
         } catch (error) {
           performanceResults.push({
     ...
     async runFullTestSuite() {
       ...
       try {
         ...
         console.log(
           `📈 Success Rate: ${((testResults.summary.passed / (testResults.summary.passed + testResults.summary.failed)) * 100).toFixed(1)}%`,
         );
   ...
   class ProductionMigrationSafety {
     ...
     async checkTableLocks(migrationFile) {
       ...
       return {
         ...
         blocking: true,
         message:
           longTransactions.rows.length > 0
     ...
     async checkDataSize(migrationFile) {
       ...
       const largeTables = tableSizes.rows.filter(
         (table) => table.size_bytes > 1000000000,
       ); // > 1GB
     ...
     async checkDependencies(migrationFile) {
       ...
       const highUsage = activeConnections.rows.filter(
         (app) => app.active_count > 10,
       );
     ...
     async checkBackupStatus(migrationFile) {
       ...
       const backupAge = backupExists
         ? lastBackup.rows[0].backup_age_seconds
         : null;
     ...
     async checkMaintenanceWindow(migrationFile) {
       ...
       const inMaintenanceWindow = maintenanceWindows.some(
         (window) =>
   ...
   class MigrationMonitor {
     ...
     async monitorMigration(migrationName, migrationFn) {
       ...
       try {
         ...
         console.log(
           `✅ Migration completed: ${migrationName} (${duration}ms)`,
         );
         return result;
       } catch (error) {
     ...
     calculateAverageExecutionTime() {
       ...
       const total = this.metrics.executionTimes.reduce(
     ...
     calculateSuccessRate() {
       const totalSuccess = Object.values(this.metrics.successCounts).reduce(
         (sum, count) => sum + count,
         0,
       );
