   ...
   **Data Validation with Joi:**
   ...
   const userSchema = {
     ...
     list: Joi.object({
       ...
       sort: Joi.string()
         .valid("id", "email", "firstName", "lastName", "createdAt")
         .default("id"),
   ...
   class UserController {
     async listUsers(req, res, next) {
       try {
         ...
         res.json(
           new ApiResponse("success", "Users retrieved successfully", {
             users: result.users,
             pagination: {
               page: result.page,
               limit: result.limit,
     ...
     async getUser(req, res, next) {
       try {
         ...
         const filteredUser = userService.filterUserData(
           user,
           requestingUserRole,
           requestingUserId,
         );
         ...
         res.json(
           new ApiResponse("success", "User retrieved successfully", {
             user: filteredUser,
           }),
         );
       } catch (error) {
     ...
     async updateUser(req, res, next) {
       try {
         ...
         res.json(
           new ApiResponse("success", "User updated successfully", {
             user: filteredUser,
           }),
         );
       } catch (error) {
     ...
     async getUserOrders(req, res, next) {
       try {
         ...
         if (
           ...
           throw new ApiError(
             403,
             "Insufficient permissions to access user orders",
           );
         }
   ...
   const options = {
     definition: {
       ...
       info: {
         ...
         version: "1.0.0",
         description:
           "A comprehensive REST API with authentication and authorization",
         contact: {
           name: "API Support",
