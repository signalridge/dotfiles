   ...
   class RateLimiter {
     ...
     setupDefaultLimiters() {
       ...
       this.authLimiter = rateLimit({
         ...
         skipSuccessfulRequests: true,
         keyGenerator: (req) =>
           `auth:${req.ip}:${req.body.email || req.body.username}`,
         message: {
     ...
     createEndpointLimiter(endpoint, config) {
       return rateLimit({
         ...
         max: config.max || 100,
         keyGenerator: (req) =>
           `endpoint:${endpoint}:${req.user?.id || req.ip}`,
         message: {
...
4. **Custom Rate Limiting Middleware**
   ...
   **Advanced Rate Limiting Middleware:**
   ...
   } = require("../rate-limiters/token-bucket");
   ...
   class AdvancedRateLimiter {
     ...
     setRateLimitHeaders(res, result, config) {
       ...
       if (config.type === "sliding-window") {
         ...
         res.set(
           "X-RateLimit-Window",
           (config.limiter.windowSize / 1000).toString(),
         );
       } else if (config.type === "token-bucket") {
   ...
   **Quota Management System:**
   ...
   class QuotaManager {
     ...
     async getUserQuota(userId, quotaType) {
       ...
       const planQuota = await this.getPlanQuota(
     ...
     async getPlanQuota(plan, quotaType) {
       ...
       const limit =
         planQuotas[plan]?.[quotaType] ||
         this.quotaTypes[quotaType].defaultLimit;
     ...
     createQuotaMiddleware(quotaType, amountFn = () => 1) {
       return async (req, res, next) => {
         ...
         const amount =
           typeof amountFn === "function" ? amountFn(req) : amountFn;
   ...
   class RateLimitAnalytics {
     ...
     async getTopBlockedEndpoints(timeRange = "24h", limit = 10) {
       ...
       for (const [limitType, data] of Object.entries(stats)) {
         for (const [endpoint, endpointData] of Object.entries(
           data.endpoints || {},
         )) {
           endpointStats.push({
             endpoint,
             ...
             ...endpointData,
             blockRate:
               endpointData.total > 0
                 ? endpointData.blocked / endpointData.total
                 : 0,
           });
         }
       }
   ...
   class RateLimitConfigManager {
     ...
     async getConfig(limiterId) {
       ...
       if (config.rows.length === 0) {
         // Use default config
         config =
           this.defaultConfigs[limiterId] || this.defaultConfigs["api:general"];
       } else {
     ...
     async updateConfig(limiterId, newConfig, userId) {
       ...
       if (!validationResult.valid) {
         throw new Error(
           `Invalid config: ${validationResult.errors.join(", ")}`,
         );
       }
   ...
   describe("Rate Limiting", () => {
     ...
     describe("User-Specific Rate Limiting", () => {
       test("should apply different limits based on user tier", async () => {
         ...
         const freeUserLimit = await findRateLimit(
           app,
           "/api/data",
           freeUserToken,
         );
         ...
         const proUserLimit = await findRateLimit(
           app,
           "/api/data",
           proUserToken,
         );
    ...
    **Rate Limiting Monitoring:**
    ...
    class RateLimitMonitor {
      ...
      async checkQuotaExhaustion() {
        ...
        for (const key of quotaKeys.slice(0, 100)) {
      ...
      calculateSummaryStats(metrics) {
        ...
        for (const limitType of limitTypes) {
          const values = metrics
            .map((m) => m.metrics[limitType])
            .filter(Boolean);

          if (values.length > 0) {
            summary[limitType] = {
              avgBlockRate:
                values.reduce((sum, v) => sum + v.blockRate, 0) / values.length,
              avgVolume:
                values.reduce((sum, v) => sum + v.requestsPerMinute, 0) /
                values.length,
      ...
      calculateTrends(metrics) {
        ...
        for (const limitType of limitTypes) {
          ...
          const lastAvg = this.calculateAverage(
            lastHour,
            limitType,
            "requestsPerMinute",
          );
