   ...
   const pageRules = [
     {
       targets: [
         {
           target: "url",
           constraint: { operator: "matches", value: "*/static/*" },
         },
       ],
       actions: [
     ...
     },
     {
       targets: [
         {
           target: "url",
           constraint: { operator: "matches", value: "*/api/*" },
         },
       ],
       actions: [
...
5. **Cache Headers and Policies**
   ...
   **Smart Cache Headers:**
   ...
   class CacheControlManager {
     static getCacheHeaders(filePath, fileType) {
       ...
       if (filePath.match(/\.(js|css|png|jpg|jpeg|gif|ico|woff2?)$/)) {
         return filePath.includes("[hash]") ||
           filePath.includes("[contenthash]")
   ...
   app.use((req, res, next) => {
     const headers = CacheControlManager.getCacheHeaders(
       req.path,
   ...
   **CDN Performance Tracking:**
   ...
   class CDNAnalytics {
     ...
     static async getCacheHitRatio() {
       ...
       return (
         data.result.totals.requests.cached / data.result.totals.requests.all
       );
     }
    ...
    **Cost Monitoring:**
    ...
    class CDNCostOptimizer {
      static async analyzeUsage() {
        ...
        if (usage.images > usage.total * 0.6) {
          recommendations.push({
            ...
            message:
              "Images account for >60% of bandwidth. Consider WebP format and better compression.",
        ...
        if (usage.cacheHitRatio < 0.8) {
          recommendations.push({
            ...
            message:
              "Cache hit ratio is below 80%. Review cache headers and TTL settings.",
      ...
      static async optimizeTierUsage() {
        ...
        const coldFiles = accessPatterns.filter(
          (file) => file.requests_per_day < 10 && file.size > 1024 * 1024, // <10 requests/day, >1MB
        );

        console.log(
          `Found ${coldFiles.length} files suitable for cold storage`,
        );
        return coldFiles;
      }
