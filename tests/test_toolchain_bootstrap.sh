...
assert_file_contains "$ROOT/README.md" 'install tools from `private_dot_config/aquaproj-aqua/aqua.yaml` (including `mise`)'
assert_file_contains "$ROOT/README.md" 'install runtimes/tools via aqua-managed `mise`'
assert_file_contains "$ROOT/README.ja.md" '`private_dot_config/aquaproj-aqua/aqua.yaml` に基づいてツール（`mise` を含む）を導入'
assert_file_contains "$ROOT/README.ja.md" 'aqua が提供する `mise` でランタイム/ツールを導入'
assert_file_contains "$ROOT/README.zh-CN.md" '根据 `private_dot_config/aquaproj-aqua/aqua.yaml` 安装工具（含 `mise`）'
assert_file_contains "$ROOT/README.zh-CN.md" '通过 aqua 提供的 `mise` 安装 runtime 与工具'
