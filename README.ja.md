...
_Nix と chezmoi で構築する、モダンで再現性の高い開発環境（macOS / Linux 対応）_
...
> [!WARNING] > **実行前に必ず確認してください！** このリポジトリにはシステム設定を変更するスクリプトが含まれます。
...
> [!IMPORTANT] > **初めて使う方へ**：`useEncryption` を聞かれたら **No**（デフォルト）を選択してください。
